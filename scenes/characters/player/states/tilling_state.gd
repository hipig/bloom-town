...
func _on_enter(actor: Node) -> void:
	...
	match actor.direction:
		...
		Vector2.DOWN:
			collision_position = Vector2(0, 9.0)
		Vector2.LEFT:
			collision_position = Vector2(-9.0, -9.0)
		Vector2.RIGHT:
			collision_position = Vector2(9.0, -9.0)

	is_tilling = true
	actor.hit_box_component.tool_type = DataTypes.ToolType.PickAxe
	actor.hit_box_component.damage = 1.0
	actor.hit_box_collision_shape.position = collision_position
	actor.animated_sprite.play("tilling_" + actor.animate_direction)
...
func _on_frame_changed(actor: Player) -> void:
	...
	if  actor.animated_sprite.frame >= 4:
		var map_layer: MapLayer = Groups.map_layer
		if map_layer:
			map_layer.tilling()
